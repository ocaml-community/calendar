...
module type S = sig
  ...
  val normalize : t -> t * int
	...
	modification.  E.g. [normalize (make 22 0 0)] returns the time 22-0-0
	and 0, [normalize (make 73 0 0)] returns the time 1-0-0 and 3 and
  ...
  val is_pm : t -> bool
	...
	[false] otherwise.
	E.g. both [is_pm (make 10 0 0)] and [is_pm (make 34 0 0)] return 
	[true]. *)

  val is_am : t -> bool
    (** Return [true] is the time is after midday in the current time zone; 
	[false] otherwise. 
	E.g. both [is_am (make 20 0 0)] and [is_am (make 44 0 0)] return 
	[true]. *)
  ...
  val prev : t -> field -> t
	...
	field.  E.g [prev (make 20 3 31) `Second] returns the time 20:03:30
	(i.e. one second ago). *)
